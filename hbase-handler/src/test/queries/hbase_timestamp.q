...
DROP TABLE hbase_table;
CREATE TABLE hbase_table (key string, value string, time string)
  STORED BY 'org.apache.hadoop.hive.hbase.HBaseStorageHandler'
  WITH SERDEPROPERTIES ("hbase.columns.mapping" = ":key,cf:string,:timestamp");
DESC extended hbase_table;
FROM src INSERT OVERWRITE TABLE hbase_table SELECT key, value, "2012-02-23 10:14:52" WHERE (key % 17) = 0;
...
explain SELECT key, value, cast(time as timestamp) FROM hbase_table WHERE time < 200000000000;
SELECT key, value, cast(time as timestamp) FROM hbase_table WHERE time < 200000000000;

explain SELECT key, value, cast(time as timestamp) FROM hbase_table WHERE time > 100000000000;
SELECT key, value, cast(time as timestamp) FROM hbase_table WHERE time > 100000000000;
