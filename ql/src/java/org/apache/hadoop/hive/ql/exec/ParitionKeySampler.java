...
public class ParitionKeySampler implements OutputCollector<HiveKey, Object> {
  ...
  private byte[][] getPartitionKeys(int numReduce) {
    ...
    Collections.sort(sampled, RandomSampler.C);
    byte[][] partitionKeys = new byte[numReduce - 1][];
    float stepSize = sampled.size() / (float) numReduce;
    int last = -1;
    for(int i = 1; i < numReduce; ++i) {
      int k = Math.round(stepSize * i);
      while (last >= k && RandomSampler.C.compare(sampled.get(last), sampled.get(k)) == 0) {
        k++;
      }
      partitionKeys[i - 1] = sampled.get(k);
      last = k;
    }
  ...
  public static FetchSampler createSampler(FetchWork work, HiveConf conf, JobConf job, Operator<?> operator) {
    ...
    float samplePercent = conf.getIntVar(HiveConf.ConfVars.HIVESAMPLINGPERCENTFORORDERBY);
