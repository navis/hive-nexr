...
public class TimestampWritable implements WritableComparable<TimestampWritable> {
  ...
  private static final DateFormat[] dateFormats = new DateFormat[7];

  static {
    for (int i = 0; i < dateFormats.length; i++) {
      dateFormats[i] = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    }
  }
  ...
  public String toString() {
    ...
    String formatted = format(timestamp.hashCode() & Integer.MAX_VALUE);
    int nanos = getNanos();
    if (nanos != 0) {
  ...
  private String format(int hash) {
    DateFormat format = dateFormats[hash % dateFormats.length];
    synchronized (format) {
